...
from sklearn.decomposition._nmf import _beta_divergence, _beta_loss_to_float
    ...
    def fit_iterative_update(self, X, Y, U, V, Z):
        ...
        for n_iter in range(1, self.max_iter + 1):
            ...
            if n_iter % 1 == 0:
                ...
                settings = [(check_sparse(U, sparse=False), [2, 0], "U"), 
                            (check_sparse(V.T, sparse=False), [1, 1], "V"), 
                            (check_sparse(Z, sparse=False), [0, 0], "Z"), 
                            (check_sparse(Y, sparse=False), [0, 2], "Y, gt"),
                            (check_sparse(X, sparse=False), [2, 2], "X, gt"), 
                            (check_sparse(X_pred, sparse=False), [2, 1], "X"),
                            (check_sparse(Y_pred, sparse=False), [0, 1], "Y")
                            ]
