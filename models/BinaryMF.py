...
class BinaryMF(BaseModel):
    ...
    '''
    def __init__(self):
        raise NotImplementedError("This is a template class. Instantiate BinaryMFPenalty or BinaryMFThreshold instead.")
    ...
    def init_model(self):
        ...
        # init U, V
        if self.init_method == 'sklearn':
            nmf = NMFSklearn(k=self.k, init='nndsvd', seed=self.seed)
            nmf.fit(X_train=self.X_train)
            # self.U, self.V = to_dense(nmf.U), to_dense(nmf.V)
            self.U, self.V = nmf.U, nmf.V
