...
from utils import to_sparse, dot, matmul, show_matrix, get_metrics, record, eval, to_triplet
    ...
    def collective_eval(self, X_gt, m, metrics, task):
        ...
        if task == 'prediction':
            ...
            if len(gt_data) < 5000:
                ...
                X_pd = self.Xs[m] # make sure it's updated
                pd_data = np.zeros(len(gt_data), dtype=int)
                for i in tqdm(range(len(gt_data)), position=0, desc="[I] Making predictions"):
            ...
            pd_data = self.Xs[m] # make sure it's updated
