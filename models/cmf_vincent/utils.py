...
def loadTripleData(filename):
	...
	# fData = scipy.sparse.coo_matrix((fData[2],numpy.int32((fData[0],fData[1])))).tocsc()
	fData = scipy.sparse.csc_matrix((fData[2],numpy.int32((fData[0],fData[1]))))
	return(fData)

def read_triple_data(train, test, user, item, feature_mat_type, boolean):
	'''
	...
	X_train = scipy.sparse.csc_matrix((Dtrain[2],(Dtrain[0],Dtrain[1])), shape=(num_user, num_item))
	X_test = scipy.sparse.csc_matrix((Dtest[2],(Dtest[0],Dtest[1])), shape=(num_user, num_item))
